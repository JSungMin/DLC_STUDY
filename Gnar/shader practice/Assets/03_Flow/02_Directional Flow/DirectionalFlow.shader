...
Shader "Custom/DirectionalFlow" {
	Properties {
		...
		[NoScaleOffset] _MainTex ("Drive {AG} Height (B)", 2D) = "black" {}
	...
	SubShader {
		...
		float3 UnpackDetrivativeHeight(float4 textureData)
		{
		...
		float3 FlowCell(float2 uv, float2 offset, float time, float gridB)
		{
			...
			float3 dh = UnpackDetrivativeHeight(tex2D(_MainTex, uvFlow));
